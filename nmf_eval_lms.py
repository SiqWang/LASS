# --------------------------------------------------------------------
# nmf_eval_lms.py  —  MUSIC dataset  Auto-K NMF + CLAP evaluation
# --------------------------------------------------------------------
import gc
...
from utils import calculate_sdr, calculate_sisdr       # Baseline utilities
# --------------------------------------------------------------------
...
def lms_filter(series, mu=0.01):
    """
    One-tap LMS smoothing on a time series (row of a mask).
    Keeps the first sample, then runs LMS to predict subsequent samples.
    Non-negativity is enforced on the output.
    """
    if len(series) < 2:
        return series
...
class AutoK_NMF_Separator:
    """
    Pipeline:
      1) Run NMF with K_max (fixed here inside separate()).
      2) (Energy thresholding column removal was noted; logic not present here.)
    ...
    def __init__(self, clap_encoder: CLAP_Encoder,
        ...
        self.loudness_norm = loudness_norm   # Optional RMS normalization per component
    ...
    def separate(self, mix: np.ndarray, q_emb: torch.Tensor) -> np.ndarray:
        ...
        # NMF(K) + sparse regularization (multiplicative updates)
        np.random.seed(0)
        F, N = mag.shape
        ...
        # Extract each separated component and score by CLAP similarity
        ys, sims = [], []
        for k in range(K):
            Vk = W[:, [k]] @ H[[k], :]
            Mk = Vk / WH

            # Per-frequency LMS smoothing for the mask row time series
            for f in range(Mk.shape[0]):
                series = Mk[f, :]
                series = lms_filter(series, mu=0.01)
                Mk[f, :] = series
...
class MUSIC_NMF_Evaluator:
    ...
    def __call__(self) -> Dict[str, float]:
        ...
        for idx, caption, *_ in tqdm(self.meta[0:5000:100]):
            ...
            """
            Example synthetic mix (kept here as reference; not executed):
            ...
            os.makedirs('AudioSep-main\\output_betanmf_lms', exist_ok=True)
            sf.write(f'AudioSep-main\\output_betanmf_lms\\{caption}_{idx}.wav', est, self.sr)
        ...
        with open('AudioSep-main\\output_betanmf_lms\\results.csv', "w", newline='') as f:
            writer = csv.writer(f)
...
def main():
    ...
    separator = AutoK_NMF_Separator(clap, K_max=4,
                                    loudness_norm=False)  # RMS normalization toggle
